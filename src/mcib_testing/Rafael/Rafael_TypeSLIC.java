 ...
 * @author thomasb
 */
public class Rafael_TypeSLIC implements ij.plugin.PlugIn {
    ...
    //ImageInt imgLabel, imgSLIC;
    ImageInt imgLabel;
    //Objects3DPopulation popSLIC;
    //ImageHandler drawSLIC;
    ...
    public void run(String arg) {
        ...
        //ImageInt imgWat = ImageInt.wrap(WindowManager.getImage("dapi-seg-SLIC-25000-wat.tif"));
...
//        }
        ...
        //imgSLIC = ImageInt.wrap(WindowManager.getImage("label.tif"));
        //popSLIC = new Objects3DPopulation(imgSLIC);
        //drawSLIC = imgSLIC.createSameDimensions();
        ...
        //this.computeTypeCellsNucleusSLIC(imgLabel, imgSLIC);
...
//    }
    /*private void computeTypeCellsNucleusSLIC(ImageInt label, ImageInt region) {
        int[] histype = {0, 0, 0, 0};
        for (Cell C : popCells) {
            Object3D nucleus = C.nucleus;
            ...
            ArrayUtil listRegion = nucleus.listValues(region);
            // check nuclei inside
            ArrayList<Object3D> nucs = new ArrayList<Object3D>();
            int nbA = 0;
            int nbB = 0;
            int nbD = 0;
            int nbU = 0;
            for (int i = 0; i < list.getSize(); i++) {
                if (list.getValue(i) == ALPHA) {
                    nbA++;
                }
                if (list.getValue(i) == BETA) {
                    nbB++;
                }
                if (list.getValue(i) == DELTA) {
                    nbD++;
                }
                if (list.getValue(i) == DAPI) {
                    nbD++;
                    int re = listRegion.getValueInt(i);
                    Object3D ob = popSLIC.getObjectByValue(re);
                    if ((ob != null) && (!nucs.contains(ob)) && (ob.pcColoc(nucleus) > 50)) {
                        nucs.add(ob);
                    }
                }
            }
            if (nbD < 0.25 * nucleus.getVolumePixels()) {
                IJ.log(nucleus + " not nucleus "+((double)nbD/(double)nucleus.getVolumePixels()));
            }
            // draw SLIC
            for (Object3D O : nucs) {
                O.draw(drawSLIC, nucleus.getValue());
            }

            C.type = UNLABELED;
            if ((nbA > nbB) && (nbA > nbD)) {
                C.type = ALPHA;
                if ((nbB > nbD) && (nbB * 2 > nbA)) {
                    IJ.log("Coloc AB " + nucleus);
                }
                if ((nbD > nbB) && (nbD * 2 > nbA)) {
                    IJ.log("Coloc AD " + nucleus);
                }
            }
            if ((nbB > nbA) && (nbB > nbD)) {
                C.type = BETA;
                if ((nbA > nbD) && (nbA * 2 > nbB)) {
                    IJ.log("Coloc BA " + nucleus);
                }
                if ((nbD > nbA) && (nbD * 2 > nbB)) {
                    IJ.log("Coloc BD " + nucleus);
                }
            }
            if ((nbD > nbA) && (nbD > nbB)) {
                C.type = DELTA;
                if ((nbA > nbB) && (nbA * 2 > nbD)) {
                    IJ.log("Coloc DA " + nucleus);
                }
                if ((nbB > nbA) && (nbB * 2 > nbD)) {
                    IJ.log("Coloc DB " + nucleus);
                }
            }
...
//        Object3D nuc0 = popCells.get(popCells.size() / 2).nucleus;
//        Object3D reg0 = popCells.get(popCells.size() / 2).region;
//        ImageHandler test = label.createSameDimensions();
//        reg0.draw(test, 255);
//        nuc0.draw(test, 0);
//        Object3DVoxels layer0 = nuc0.getLayerEVFObject((ImageInt) test, 0.1f);
//        test.draw(layer0, 128);
//        test.show("test");
    ...
    private void computeTypeCellsNucleusSLIC(ImageInt label) 
    {
        ...
        unLabelledCell.show("Unlabelled Cells");
        unLabelledCell.setTitle("unlabelled cells");
        //unLabelledCell.save(dir);
        double[] y = {0, 1, 2, 3};
        PlotWindow.noGridLines = false; // draw grid lines
        Plot plot = new Plot("Example Plot","X Axis","Y Axis", y, histype);
