 ...
 * @author thomasb
 */
public class Rafael_Type implements ij.plugin.PlugIn {
...
//                                        }
    ...
    private void initCells(ImageInt nucLabel, ImageInt regionLabel) {
        ...
        for (Object3D region : popRegions.getObjectsList()) {
            ...
            Cell cell = new Cell();
            cell.region = region;
            Object3D nuclO = popNuclei.getObjectByValue(nuc);
            if (nuclO != null) {
                cell.nucleus = nuclO;
                popCells.add(cell);
                cell.id = c++;
    ...
    private void computeTypeCellsLayer(double threshold) {
        ...
        for (Cell C : popCells) {
            ...
            if (ids.getValue(idx[idx.length - 1]) < 2 * ids.getValue(idx[idx.length - 2]) && (ids.getValue(idx[idx.length - 2]) > threshold)) {
                //C.type = UNLABELED;
                nucleus.setComment("*");
                IJ.log("coloc " + C.nucleus.getValue() + " : " + (idx[idx.length - 1] + 1) + " " + ids.getValue(idx[idx.length - 1]) + " " + (idx[idx.length - 2] + 1) + " " + ids.getValue(idx[idx.length - 2]));
            }
