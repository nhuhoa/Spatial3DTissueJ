 ...
 * @author tranhoa
 */
public class RegionSLIC_V4 implements ij.plugin.PlugIn
{
...
//        } 
    ...
    public void testNuc()
    {
        ...
        for (int k = 0; k < popCells.size(); k++) 
        {
            ...
            for (int i = 0; i < ac1.getSize(); i++) 
            {
                ...
                {    
                    if(popR.getObjectByValue(indexRegion)!=null)
                    {
                        ...
                        {
                            count++;
                            mapR.setValue("idx"+count, C.id, indexRegion);
                            //nbNuc++; 
                            vNuc +=vol;
                            re.draw(labelNuc, 80);
            ...
            mapR.setValue("vol SLIC", C.id, vNuc);
            mapR.setValue("vol SEG", C.id, volNuc);
            if(vNuc < volNuc * 0.4){
                mapR.setValue("status", C.id, 0);
            }
            else{
                mapR.setValue("status", C.id, 1);
                countC++;
            }
        }
        IJ.log("Result: nb of good result: "+countC + " nb bad result: "+ (nbCells-countC)+" probab %: "+ (countC*100/nbCells));
        for (Cell C : popCells)
        {
            Object3D nuc = C.nucleus;
...
//            region.setLabelImage(null);
    ...
    public void createTestImage()
    {
        ...
        for(Cell C : popCells)
        {
            ...
            nuc.draw(imgBorder, 255);
        }
        ImageInt mark = new ImageByte("label", imgSeg.sizeX, imgSeg.sizeY, imgSeg.sizeZ);
        for (int z = 0; z < mark.sizeZ; z++) 
        {
            for (int y = 0; y < mark.sizeY; y++) 
            {
                for (int x = 0; x < mark.sizeX; x++) 
                {
                    if(imgBorder.getPixel(x, y, z) != 0)
                    {
                        mark.setPixel(x, y, z, 255);
                    }    
                }
            }
        }    
        ...
        imgBorder.show("res normalise");
        
        for (int k = 0; k < popCells.size()/30; k++) 
        {
            Cell C = popCells.get(k);
            Object3D r = C.region;
            ImageInt labelWat = new ImageByte("label Nuc", imgSeg.sizeX, imgSeg.sizeY, imgSeg.sizeZ);
            r.draw(labelWat, 255);
            ImageInt labelTest = new ImageByte("label Nuc", imgSeg.sizeX, imgSeg.sizeY, imgSeg.sizeZ);
            labelTest = mark.duplicate();
            labelTest.intersectMask(labelWat);
            ImageHandler imgRes = dapi.duplicate();
            imgRes.intersectMask(labelTest);
            imgRes.setTitle(C.nucleus.getValue()+"");
            imgRes.save(dir+"/test/");
...
//        } 
        ...
        try {
            FileWriter ryt = new FileWriter("/home/tranhoa/Raphael/data_test/testV2/Islet1-3B-small/SEG/v5/mapping2.arff");
        ...
        try {
            ...
            Logger.getLogger(RegionSLIC_V4.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void mappingRCInsideNuclei()
    {
        ...
        ResultsTable cellM = new ResultsTable();
        try {
            FileWriter ryt = new FileWriter("/home/tranhoa/Raphael/data_test/testV2/Islet1-3B-small/SEG/v6/mappingRCNuc3.arff");
            BufferedWriter out=new BufferedWriter(ryt);
            out.write("@relation cell\n");
            out.write("@attribute nbA numeric\n");
            out.write("@attribute nbB numeric\n");
